...
12. **Remember the fork changelog before every push** - before pushing commits that add or change intentional fork behavior, read and update `fork_changelog.md`. Commit the implementation first, record its full hash, parent/base, decisions, affected areas, and verification in a separate changelog commit, then push both. Do not add an entry for the changelog-only follow-up commit itself; that would create an infinite self-reference loop. Preserve unrelated uncommitted work while doing this.
