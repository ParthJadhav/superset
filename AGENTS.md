...
12. **Update the fork changelog after every behavior commit** - after each commit that adds or changes intentional fork behavior, read and update `fork_changelog.md` immediately; do not defer this until push time. Commit the implementation first, record its full hash, parent/base, decisions, affected areas, and verification in a separate changelog commit, then push both when requested. Do not add an entry for the changelog-only follow-up commit itself; that would create an infinite self-reference loop. Preserve unrelated uncommitted work while doing this.
13. **Test changes in the development version** - after every code or UI behavior change, test the exact changed journey in the development build for the current worktree before committing. For desktop UI changes, follow the CDP evidence gate above: verify the correct renderer, signed-in session, route, real user input, lifecycle, and screenshots/measurements. If development verification is blocked, do not substitute another app instance or claim end-to-end success; record the blocker and the narrower automated checks that passed.
